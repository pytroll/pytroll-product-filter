...
"""Definition of classes and functions used throughout the package."""
...
class GranuleFilter(object):
    ...
    def __call__(self, message):
        ...
        for areaid in self.areaids:
            ...
            inside = self.granule_inside_area(
    ...
    def granule_inside_area(
        self, start_time, end_time, platform_name, area_def, tle_file=None
    ):
        ...
        try:
            ...
            LOG.exception("Failed getting orbital data for {0}".format(platform_name))
            LOG.critical(
...
def granule_inside_area(
    ...
    mypass = Pass(
        platform_name, start_time, end_time, instrument=instrument, tle1=tle1, tle2=tle2
    )
    acov = mypass.area_coverage(area_def)
    LOG.debug("Granule coverage of area %s: %f", area_def.area_id, acov)
